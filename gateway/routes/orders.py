...
def get_settings_dep():
    from main import get_settings_dep as _get
...
async def create_order(
    ...
    try:
        ...
        # map unexpected storage errors
        raise HTTPException(status_code=400, detail=str(e)) from e

    order_id = created["id"]
    bg.add_task(messaging.publish_order, order_id, order.model_dump(mode="json"))
...
async def order_events(
    ...
    resp = EventSourceResponse(event_gen(), ping=settings.sse_ping_seconds)
