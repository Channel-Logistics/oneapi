...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "bbox", pg.ARRAY(sa.Numeric()), nullable=True
        ),  # si prefieres double precision[] usa pg.ARRAY(sa.Float())
    ...
    op.create_table(
        ...
        sa.Column(
            "data", pg.JSONB(), nullable=True, server_default=sa.text("'{}'::jsonb")
        ),
        sa.Column(
            "created_at",
            pg.TIMESTAMP(timezone=True),
    ...
    op.create_table(
        ...
        sa.Column(
            "meta", pg.JSONB(), nullable=True, server_default=sa.text("'{}'::jsonb")
        ),
    )
    ...
    op.create_index(
        "ix_events_order_id_created_at", "events", ["order_id", "created_at"]
    )
    op.create_index("ix_order_providers_order_id", "order_providers", ["order_id"])
    op.create_index(
        "ix_order_providers_provider_id", "order_providers", ["provider_id"]
    )
